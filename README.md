...
| **Portal intake** | Vercel → Settings → Environment Variables | Each enquiry is also forwarded to the Royto platform (`royto-app`) as a lead, after the email is sent. Needs `ROYTO_INTAKE_URL` (`https://app.royto.tech/api/intake/leads`) and `ROYTO_INTAKE_KEY` (the Royto organization's intake key, shown on the portal's Web & lead capture page). Unset = nothing forwarded, form unaffected. |
...
| Lighthouse `/` mobile | **Against production (royto.tech, Vercel edge, 12 Sep 2026): Perf 96–98 across three runs, LCP 2.4–2.6 s · A11y 100 · Best Practices 100 · SEO 100.** Locally (`next start`) the same page reads 93–95 — the local server is the difference, not the page. Note for local runs: with Vercel Web Analytics in the layout, a local `next start` logs a 404 for `/_vercel/insights/script.js` (the script only exists on Vercel) and Best Practices reads 96; on production it is 100 with no console errors. The LCP element is the nav wordmark with a ~2.6 s simulated render delay — the framework floor below, not page content — so the score moves with run-to-run noise rather than with what is on the page. |
| Lighthouse `/` desktop | Perf 100 · A11y 100 · Best Practices 100 · SEO 100 |
| Interior pages, mobile | Against production: 95–97 Perf (about 97, packages 97, pricing 97, what-we-automate 97, never-miss-a-lead 95), 100 across the other three. Locally 93–94. A first run of /packages and /pricing after the navigation pass read 90 and 91 with 140–230 ms TBT; three re-runs each read 93 and 94 with 50–90 ms — the first run was a cold-start outlier, and the on-this-page bar's scroll listener costs nothing measurable. `/pricing` sits at 93 after the restructure added the Royto Social block and the example audit map; converting its Included section from Motion to the CSS reveal did not move the score — the LCP is the framework floor described below, not app animation. `/contact` is server-rendered on demand (it reads `?for=`) |
